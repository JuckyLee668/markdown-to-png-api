 ...
 * Markdown 转 PNG 转换器
 */
export class MarkdownToPngConverter {
   ...
   * 加载 CSS 文件内容
   */
  private loadCSS(cssPath?: string): string {
   ...
   * 生成完整的 HTML 文档（包含样式）
   */
  private generateFullHtml(markdownHtml: string, options: ConvertOptions): string {
 ...
 */
export async function convertMarkdownToPng(
  markdown: string,
  outputPath?: string
): Promise<string> {
